    ...
    const onSubmit = async (guest: any) => {
        try {
            ...
            if (!guest) {
                ...
                if (!values.name) {
                    showMessage({
                        type: "danger",
                        message: "Error",
                        description: "El nombre es requerido para invitado",
                    });
                    return;
                }
                if (!image) {
                    showMessage({
                        type: "danger",
                        message: "Error",
                        description: "La foto es requerida para invitado",
                    });
                    return;
                }
            }
            ...
            if (!guest) {
                ...
                await sendPushNotification({
                    title: "Registro",
                    description: "Se registró un nuevo invitado",
                    profile: ["supervisor", "admin"],
                });
                await sleep(1000);
                if (userData.user.email === undefined) {
